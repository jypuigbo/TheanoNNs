...
class HebbianAdaptiveLayer(object):
	...
	def getUpdateParams(self):
		...
		# Update state
		update.append( (self.params[0], self.output) )
...
out = [LIV.output,LII_III.output]#, layer1.output]

inp = T.matrix()
#propagate = theano.function([out],y)

updates = LIV.getUpdateParams() #+ layer0.getUpdateParams()
...
#def updat(event):
	...
	orig_in.set_data(LIV.output.get_value().toarray().reshape((51,5),order='C'))
	...
	#print LIV.weights[0].get_value().toarray()
	w.set_data(LIV.weights[0].get_value().toarray())
	out_plot.set_data(outp[1].toarray().reshape(L0_shape))
	#w.set_data(Wi)
	fig.canvas.draw()
