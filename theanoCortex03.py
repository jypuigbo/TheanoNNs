...
def kernel2connection(k,inp, out):
...
layer0 = HebbianAdaptiveLayer(layer0_input,filter_shape,sigma,input_shape, L0_shape)
...
orig_in = ax[1].imshow( np.zeros((input_shape[0],1)) ,vmin = 0, vmax = 1, interpolation='none',aspect='auto')
...
def something(x,logs):
	print x.shape
	print logs.shape
	aux = (np.log(x-logs))/np.log((1-logs))
...
#def updat(event):
	...
	orig_in.set_data(fft_input.reshape(input_shape[0],1,order='C'))
